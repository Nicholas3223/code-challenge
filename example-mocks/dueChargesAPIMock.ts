...
export function MOCK_PUT_UPDATE_DUE_CHARGES(accountId: string, paymentAmount: number): Promise<DueCharges[]> {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      for (const charge of accountCharges) {
        ...
        const reduction = Math.min(charge.amount, remainingAmount);
